    ...
    if (isMonthView) {
                ...
                <div className="flex flex-col border-t border-l border-border rounded-lg bg-card shadow-inner overflow-hidden">
                    <div className="grid grid-cols-7 bg-muted">
                        {dayHeaders.map(header => (
                            <div key={header} className="p-2 text-center text-sm font-semibold text-primary border-r border-b border-border/30">
                                {header}
                            </div>
                        ))}
                    </div>
                    {calendarWeeks.map((week, weekIndex) => (
                        <div key={weekIndex} className="grid grid-cols-7 h-full">
                            {week.map((day, dayIndex) => {
                                if (!day) {
                                    return <div key={`empty-${weekIndex}-${dayIndex}`} className="border-r border-b border-border/30 bg-muted/40 min-h-[140px]"></div>
                                }
                                const cellKey = `${employee.id}-${day.fullDate.toISOString().split("T")[0]}`;
                                const cellTasks = assignments[cellKey] || [];
                                return (
                                    <div key={day.fullDate.toISOString()} className="border-r border-b border-border/30 min-h-[140px] flex flex-col relative">
                                        <span className={cn("p-1 text-xs font-semibold", day.isToday ? "bg-primary text-primary-foreground rounded-full w-5 h-5 flex items-center justify-center m-1" : "text-muted-foreground")}>
                                            {day.dayOfMonth}
                                        </span>
                                        <PlanningGridCell
                                            tasks={cellTasks}
        ...
        gridContent = (
            <div className="flex-grow overflow-auto rounded-lg border border-border bg-card shadow-inner">
                <div className={cn("grid sticky top-0 bg-muted z-20 border-b-2 border-border", gridColsClass)}>
                    ...
                    {displayDates.map((day) => (
                        <div key={day.fullDate.toISOString()} className={cn("p-1 md:p-3 text-[10px] md:text-sm font-semibold text-primary text-center border-r border-border/30 last:border-r-0", day.isToday && "bg-primary/10 text-primary-foreground font-bold rounded-t-lg")}>
                            <span className="block text-xs md:text-sm capitalize">{day.dayName}.</span>
                            <span className="block text-lg md:text-xl font-bold">{day.dayOfMonth}</span>
                            {(day.fullDate.getDate() === 1 || !isMonthView) && (<span className="block text-xs md:text-sm text-muted-foreground capitalize">{day.monthName}.</span>)}
                        </div>
                    ))}
                </div>
                <div>
                    {employees.map((employee) => (
                        <div className={cn("grid", gridColsClass)} key={employee.id}>
                            <div className="p-1 md:p-2 border-r border-b border-border/30 flex flex-col items-center justify-center text-center bg-muted/40 sticky left-0 z-10">
                                <span className="text-[10px] md:text-sm font-medium text-secondary-foreground truncate w-full">{employee.nombre}</span>
                            </div>
                            {displayDates.map((day) => {
                                const cellKey = `${employee.id}-${day.fullDate.toISOString().split("T")[0]}`;
                                const cellTasks = assignments[cellKey] || [];
                                return (
                                    <motion.div key={cellKey} className="relative h-full">
                                        <PlanningGridCell tasks={cellTasks} employeeId={employee.id} day={day} onRemoveTask={onRemoveTask} onShowTaskDetails={onShowTaskDetails} onEmptyCellClick={handleEmptyCellClick} employeesList={employees} />
                                    </motion.div>
                                );
                            })}
                        </div>
