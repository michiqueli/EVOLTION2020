...
import { UploadCloud, FileImage as ImageIcon, Trash2, Loader, CheckCircle, XCircle } from 'lucide-react';
        ...
        {filePreviews.map((preview) => (
          ...
          >
            <div className="flex items-center gap-2 truncate">
              {preview.localPreview && <img  src={preview.localPreview} alt={preview.name} className="h-8 w-8 rounded object-cover" />}
              <span className="truncate text-foreground" title={preview.name}>{preview.name}</span>
            </div>
            <div className="flex items-center gap-1">
              {preview.status === 'uploading' && <Loader className="h-4 w-4 animate-spin text-primary" />}
              {preview.status === 'uploaded' && <CheckCircle className="h-4 w-4 text-green-500" />}
              {preview.status === 'error' && <XCircle className="h-4 w-4 text-destructive" />}
              <Button type="button" variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-destructive" onClick={() => removeFile(preview.id)} disabled={(isUploading && preview.status === 'uploading') || disabled}>
